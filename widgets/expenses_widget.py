from widgets.dialogs.add_expense_dialog import AddExpenseDialog
...
class ExpensesWidget(QWidget):
    def __init__(self):
        ...
        expenses_label.setFont(QFont("Arial", 25, 2, False))
        v_layout.addWidget(expenses_label, 0, Qt.AlignmentFlag.AlignCenter)
        ...
        add_push_button.clicked.connect(self.OpenAddExpenseDialog)
        add_push_button.setFont(QFont("Arial", 16, 2, False))
        remove_push_button = QPushButton("Remove")
        remove_push_button.clicked.connect(self.RemoveButtonClicked)
    ...
    def OpenAddExpenseDialog(self):
        expense_dialog_box = AddExpenseDialog(self)

        if expense_dialog_box.exec_() == 1: # 1=accepted
            print("Accepted")
            data = expense_dialog_box.get_data()
    ...
    def UpdateDataList(self, data):
        ...
        self.table_widget.setItem(self.table_widget.rowCount() - 1, 2, QTableWidgetItem(str(data['Expense Type']), 0))
        self.table_widget.setSortingEnabled(True)